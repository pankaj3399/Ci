...
const SearchListing = {
  ...
  async resolve({ request }, {
    ...
    if (dates && dates.toString().trim() !== '') {
      let noticeFilter = [], spiltDate = dates.split("AND"), checkIn, checkOut, startDay, endDay, currentDay, dayDifference;
      ...
      // Date Range Filter
      if (dates && dates.toString().trim() != '') {
        let splitDates, startDate, endDate, searchDate;
        splitDates = dates.split("AND");
        startDate = splitDates && splitDates.length > 0 && splitDates[0] && splitDates[0].replace(/'| /g, "");
        endDate = splitDates && splitDates.length > 0 && splitDates[1] && splitDates[1].replace(/'| /g, "");
