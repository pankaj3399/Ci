...
const updateClaimRefund = {
    ...
    async resolve({ request }, { reservationId, claimRefund }) {
        try {
            if (!request?.user || !request?.user?.admin) return { status: 500, errorMessage: await showErrorMessage({ errorCode: 'userLoginExist' }) };
            ...
            if (!reservation) return { status: 400, errorMessage: await showErrorMessage({ errorCode: 'invalidError' }) };
            ...
            if (claimRefund > 0) {
                ...
                if (!transactionData) return { status: 400, errorMessage: await showErrorMessage({ errorCode: 'invalidError' }) };
