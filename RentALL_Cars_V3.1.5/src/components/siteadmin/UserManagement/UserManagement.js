...
class UserManagement extends React.Component {
  ...
  handleSearchChange = (e) => {
   ...
   deleteChange=async(id, profileId, userType) =>{
    const { deleteUser } = this.props;
    const { userManagement: { refetch, userManagement }, setStateVariable, currentPage } = this.props;
    let variables = { currentPage: 1 && userManagement?.usersData?.length == 1 ?  currentPage - 1 : currentPage };
   const response = await deleteUser(id, profileId, userType);
