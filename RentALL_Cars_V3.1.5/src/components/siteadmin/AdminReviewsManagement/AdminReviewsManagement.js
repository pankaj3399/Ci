...
class AdminReviewsManagement extends React.Component {
  ...
  deleteReview = async (id) => {
    const { deleteAdminReview, setStateVariable, currentPage } = this.props;
    const { reviewsManagement: { refetch, reviewsManagement } } = this.props;
    let variables = { currentPage: currentPage > 1 &&  reviewsManagement?.reviewsData?.length == 1 ? currentPage - 1 : currentPage }
      ...
      <div className={cx(s.pagecontentWrapper)}>
        <div className={s.contentBox}>
          <CommonTable
            thead={this.thead}
            tbody={this.tbody}
            title={formatMessage(messages.adminManageReviews)}
            isSearch
