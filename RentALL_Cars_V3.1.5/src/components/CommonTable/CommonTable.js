        ...
        return (
            <div>{!isHeadingHide && <h1 className={cx(cs.commonTotalText, cs.spaceBottom5, cs.fontWeightBold)}>{title}</h1>}
                <div className={cx(cs.dFlexContainer, cs.mobileTableSearch, cs.flexEnd, 'mobileTableSearchRTL')}>
                    ...
                    <div>
                        {
                            isLink && href && <Link to={href} className={cx(cs.dFlex, cs.spaceBottom5, cs.commonLinkBorder, cs.noTextDecration, cs.commonContentText, cs.fontWeightMedium, cs.siteLinkColor, cs.textNoWrap)}>
                                <img src={tickIcon} className={cs.csvImageSpace} /> {redirectionLabel}
                            </Link>
                        }
                        {
                            isLink && addAction && <Link noLink onClick={addAction} className={cx(cs.dFlex, cs.spaceBottom5, cs.commonLinkBorder, cs.noTextDecration, cs.commonContentText, cs.fontWeightMedium, cs.siteLinkColor, cs.textNoWrap)}>
                                <img src={tickIcon} className={cs.csvImageSpace} /> {redirectionLabel}
                            </Link>
                        }
                        {
